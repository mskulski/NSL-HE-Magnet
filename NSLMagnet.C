...
{	
	...
	while(1)
	{
		...
		{	if(per1!=0 || A1==m0)
			...
			m0in=icount;}
			icount++;}	}
		if(name1==el_add && A1==m_add){m_add=massi*mu;}
		if(!itable.good()){break;}
	}
	itable.close();
	...
	{	if(s==m0in){continue;}
		if(abun[s]<amin){amin=abun[s];}
		if(abun[s]>amax){amax=abun[s];
						m0max=s;}
	}
	...
	for(int j=0;j<n;j++)
	{
	Double_t y0 = r3->Gaus(0,0.002);
	Double_t x0 = r3->Gaus(0,0.002);
	if(ab(y0)>0.005 || ab(x0)>0.00075){continue;}
	Double_t phi0 = r3->Gaus(pi()/2,0.001);
	Double_t theta0 = r3->Gaus(pi()/2,0.001);
	Int_t vcount = 0;
	...
	Double_t f = q*B/mp;
	zcalc(mp,v0,theta0,phi0,f,x0,y0,q,s,j,m0);
	}
	ntot++;
	}
	...
	mass1[s]->SetLineColorAlpha(1,1);
	mass12[s]->SetFillColorAlpha(18+5*s,0.8);
	mass12[s]->SetLineColorAlpha(1,1);
	if(s==0)
	{mass12[0]->SetStats(kFALSE);
	mass12[0]->GetXaxis()->SetTitle("Magnet Exit/Max Position (cm)");
	...
	for(int s=0;s<icount;s++)
	{
	mass1[s]->SetAxisRange(0,1.25*maxy,"Y");
	mass12[s]->SetAxisRange(0,1.25*maxy,"Y");
	}
	...
	mass123[s]->SetLineColorAlpha(1,1);
...
void zcalc(Double_t mc,Double_t vc,Double_t thetac,Double_t phic,Double_t fc,Double_t xc,Double_t yc,Double_t qi,Int_t v,Int_t j1,Double_t m0i)
{
	Double_t mc = m[v];
	...
	Double_t vp = ab(vc*sn(thetac));
	Double_t vy = vc*cs(thetac);
	Double_t vx0 = vc*sn(thetac)*cs(phic);
	Double_t t0 = -asn(vx0/vp)/fc;
	Double_t L = 2*1.016;

	Double_t p1 = vp/fc;
	Double_t p2 = tn(am);
  Double_t p31 = 3.048 - 1.016*(1+tn(am)+tn(phic)) + xc*tn(phic);
  Double_t p32 = p31/(tn(phic)+tn(am));
...
void Vcalc(Double_t mc,Double_t vc,Double_t thetac,Double_t phic,Double_t fc,Double_t xc,Double_t yc,Double_t qi, Int_t v, Double_t m0i)
{
	Double_t mc = m[v];
	Double_t K = 0.5*mc*vc*vc;
	Double_t Vc = 0;
	...
	vc = sqrt(2*K/mc)*sqrt(1 + qi*Vc/K);
	Double_t vp = ab(vc*sn(thetac));
	Double_t vy = vp*cs(thetac);
	Double_t vx0 = vp*cs(phic);
	Double_t t0 = asn(vx0/vp)/fc;
	Double_t L = 2*1.016;
	Double_t x1 = xc + L*tn(pi()/2-phic);
	Double_t y1 = yc + L*tn(pi()/2-thetac);
	Double_t p1 = vp/fc;
	Double_t p2 = tn(am);
	Double_t p3 = (3.048-1.016*(1+tn(am))-(1.016-xc)*tn(phic))/(tn(phic) + tn(am)) + p1*cs(-fc*t0);
	Double_t p4 = -((3.048-1.016*(1+tn(am))-(1.016-xc)*tn(phic))/(tn(phic) + tn(am)))*tn(am) - 1.016*(1 + tn(am)) - p1*sn(-fc*t0);
	Double_t oo = 2*(atn((sqrt(p1*p1*p2*p2 + p1*p1 - p2*p2*p4*p4 - 2*p2*p3*p4 - p3*p3) - p1*p2)/(p1 + p2*p4 + p3)));
	Double_t tm = (oo/fc)+t0;
